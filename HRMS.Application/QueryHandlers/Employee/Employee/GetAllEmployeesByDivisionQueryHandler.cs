...
{
    public class GetAllEmployeesByDivisionQueryHandler : IRequestHandler<GetAllEmployeeByDivisionQuery, List<EmployeeModel>>
    {
        private readonly IEmployeeRepository _repo;

        public GetAllEmployeesByDivisionQueryHandler(IEmployeeRepository repo)
        {
            _repo = repo;
        }

        public async Task<List<EmployeeModel>> Handle(GetAllEmployeeByDivisionQuery request, CancellationToken cancellationToken)
        {
            var models = await _repo.GetAllEmployeesByDivAsync(request.Id);
            return models
